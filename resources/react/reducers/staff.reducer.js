...
export const staffAsyncActions = {
  ...
  adminCreateStaff: createAsyncThunk("user/adminCreateStaff", async (payload, thunk) => {
    try {
      const api = new AuthStaffApi();
      const response = await api.post(API_ENDPOINTS.adminCreateStaff, decamelizeKeys(payload));
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(staffAsyncActions.getAllStaffs.fulfilled, (state, action) => {
      action.payload.data = action.payload.data.map((staff) => ({
        ...staff,
        createdAt: dayjs(staff.createdAt).format("YYYY-MM-DD"),
      }));
