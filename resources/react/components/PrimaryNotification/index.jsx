...
export const PRIMARY_NOTIFICATION_CONTAINER_ID = "primary-notification-container-id";

export const PrimaryNotificationComponent = () => {
  const primaryNotification = useSelector((state) => state.common.primaryNotification);
  ...
  useEffect(() => {
    if (primaryNotification) {
      if (isJsonString(primaryNotification)) {
        const notifications = JSON.parse(primaryNotification).map((notification) => ({
          ...notification,
          isOpen: true,
        }));
        ...
        setNotifications((state) => [...state, { ...primaryNotification, isOpen: true }]);
