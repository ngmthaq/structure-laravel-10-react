...
export const handleRejectedExtraReducerFormNotification = (state, action) => {
  if (action.payload.status === 403) {
    state.primaryNotification = PrimaryNotificationModel("error", __("custom.access-deny"));
  } else if (action.payload.status === 422) {
    const notifications = Object.values(action.payload.data.errors).map((error) =>
    ...
    state.primaryNotification = PrimaryNotificationModel("error", __("custom.something-wrong"));
  }
};
